 ...
 *    ==> 선언은 되어 있으나 코드가 구현되지 않은 껍데기만 있는
 *        메서드를 말함.
 *        예) void display();    // {    } 가 없다.
 ...
 * 9. 용도 : - 응용 프로그램에서 설계와 구현 부분을 분리해서 
 *            작업 시 사용됨.
 *          - 추상 클래스를 책의 목차에 비유를 하면, 
 *            서브클래스는 목차에 따라 완성된 책과 같음. 
 *            책을 쓸 때도 목차를 잡아 놓고 책을 쓰면 훨씬 
 *            쉽고 빠르며 방향이 흐트러지지 않는 것처럼 
 *            추상 클래스를 이용하면 응용 프로그램의 설계와 
 *            구현을 분리할 수 있음.
 */
