 ...
 * 인터페이스(Interface)?
 * 1. 완벽한 추상화를 제공한다.
 *    ==> - 일종의 추상클래스. 하지만 추상클래스보다 추상화 정도가
 *          더 높다.
 *        - 실제 구현된 것이 전혀 없는 기본 설계도(알멩이 없는 껍데기).
 *        - 추상메서드(알멩이 없는 껍데기)와 상수만을 멤버로 갖는다.
 *        - 인스턴스를 생성할 수 없고, 클래스 작성에 도움을 줄 목적으로
 *          사용이 됨.
 *        - 미리 정해진 규칙에 맞게 구현하도록 표준을 제시하는데 사용이
 *          됨 ==> 명세서.
 ...
 *     
 * ※ 인터페이스 사용 목적 : 스펙을 주어 클래스들이 그 기능을 서로
 *                      다르게 구현할 수 있도록 하는 클래스의
 *                      규격 선언이며, 클래스의 다형성을
 *                      실현하는 도구임.
 * [인터페이스 장점]
 * 1.개발 시간을 단축할 수 있음. 
 ...
 *                 
 */
public interface Inter { // 일반적인 멤버메서드가있으면 인터페이스 클래스를 만들 수 가없다.
